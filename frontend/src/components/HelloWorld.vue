<template>
  <div class="flex flex-col justify-center items-center min-h-screen">
    ...
    <div class="p-8 flex flex-col items-center">
      ...
      <input v-model="url" placeholder="Digite a URL" class="border p-2 w-full mb-2 rounded-sm" />
      <span v-if="error" class="text-lg dark:text-red-300 mb-4">{{ error }}</span>
      
      <div>
        <button @click="encurtar" type="button" class="bg-blue-500 text-white px-4 py-2 rounded hover:cursor-pointer hover:bg-blue-700 mr-4">Encurtar</button>
        <button v-if="url || resultado" @click="limpar" type="button" 
          class="bg-yellow-500 text-white px-4 py-2 rounded hover:cursor-pointer hover:bg-yellow-700"> 
          Limpar
        </button>
      </div>
